...
TEAMS_RESTING_STARTERS = [] 

# ==========================================
# TEAM MAPPING (Global)
# ==========================================
# Maps Full Name -> Abbreviation
TEAM_MAP = {
...
# Reverse Map: Abbreviation -> Full Name (For Database)
...
def get_live_odds():
    ...
    try:
        ...
        for game in res:
            ...
            if game_dt > max_future or game_dt < now: continue
            ...
            matchups.append({
                "home": h, "away": a, "spread": float(spread), "total": float(total), 
                "book": book_used, "time": local.hour, "thurs": 1 if local.weekday() == 3 else 0,
                "date_iso": game_dt.isoformat()
            })
...
def save_predictions_to_db(predictions):
    ...
    try:
        ...
        INSERT INTO "predictions_2.0" (
            ...
            spread, total, confidence, sport_id, game_id, 
        ...
        for p in predictions:
            # game_id generation (Using Season + Week + Teams to be unique)
            game_id_str = f"{CURRENT_SEASON}_WEEK_18_{p['away']}@{p['home']}"
            
            # Use safe defaults if values are missing
            conf_val = conf_map.get(p['confidence'], "low") 
            
            cur.execute(insert_query, (
                'NFL',              # league
                p['home_full'],     # home_team (NOW FULL NAME)
                p['away_full'],     # away_team (NOW FULL NAME)
                p['date_iso'],      # date (timestamp)
                p['pick_team'],     # predicted_winner (NOW FULL NAME)
                p['pick_text'],     # predicted_spread (NOW FULL NAME FORMAT, MATCHING PICK COL)
                str(p['con_total']),# predicted_total (e.g. "46.5")
                str(p['vegas_spread']), # spread (Vegas)
                str(p['vegas_total']),  # total (Vegas)
                conf_val,           # confidence (enum)
                2,                  # sport_id (Default 2 for NFL)
                game_id_str         # game_id (Unique Key)
            ))
        
        conn.commit()
        print("   [DB] ✅ Successfully saved/updated predictions.")
        cur.close()
    except Exception as e:
...
def run_consensus():
    ...
    injury_audit = []
    db_payload = [] # List to store rows for DB
    ...
    for m in matchups:
        ...
        # 2. Predicted Winner Name (Full)
        predicted_winner_abbr = home if con_raw > 0 else away
        predicted_winner_full = ABBR_TO_FULL.get(predicted_winner_abbr, predicted_winner_abbr)
        
        # 3. Pick Text (Full Name Format) - MATCHING PICK COLUMN LOGIC
        if diff > 0: 
            # Logic: f"{home} {spread}" -> "Full Name Spread"
            pick_text_full = f"{home_full} {spread}"
        else:
            # Logic: f"{away} +{abs(spread)}" if spread < 0 else f"{away} {spread}"
            if spread < 0:
                pick_text_full = f"{away_full} +{abs(spread)}"
            else:
                pick_text_full = f"{away_full} {spread}"
                
        # --- Standard Pick for Console (Abbr) ---
        ...
        db_payload.append({
            "home": home,
            "away": away,
            "home_full": home_full,             # Full Name
            "away_full": away_full,             # Full Name
            "date_iso": m.get("date_iso"),
            "pick_team": predicted_winner_full, # Full Name
            "pick_text": pick_text_full,        # Full Name Spread Text
            "con_spread": round(con_raw, 1),
